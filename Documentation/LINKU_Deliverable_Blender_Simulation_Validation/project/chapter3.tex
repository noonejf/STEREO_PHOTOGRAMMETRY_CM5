...
Table \ref{tab:chosen-camera-intrinsics} lists the resulting camera intrinsics. These are the values the Blender camera object should be configured with once this sensor replaces the IMX477 assumption currently hardcoded in \texttt{config/camera\_config.py} and \texttt{camera/stereo\_camera.py} (a separate, still-pending software adaptation task).
...
\todo[inline]{The 1.5 m minimum object distance above is taken from the fixed-focus B0308 product page directly; it was not independently bench-verified. Confirm once the physical unit is in hand, since this affects whether the near-field dark-enclosure captures (well under 1.5 m) will be in focus.}
